<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:74:"D:\www\twothink\public/../application/admin/view/default/baoxiu\index.html";i:1506736108;s:73:"D:\www\twothink\public/../application/admin/view/default/public\base.html";i:1496373782;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
			...
			<tbody>
				<?php if(!(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty()))): if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$baoxiu): $mod = ($i % 2 );++$i;?>
					<tr>
						...
						<td><?php echo $baoxiu['sn']; ?></td>
						<td><?php echo $baoxiu['name']; ?></td>
						<td><?php echo $baoxiu['tel']; ?></td>
                        <td><?php echo $baoxiu['detail']; ?></td>
                        <td><?php echo $baoxiu['status']; ?></td>
						<td>
							<a title="编辑" href="<?php echo url('edit?id='.$baoxiu['id']); ?>">编辑</a>
							<a href="<?php echo url('setStatus?ids='.$baoxiu['id'].'&status='.abs(1-$baoxiu['status'])); ?>" class="ajax-get"><?php echo show_status_op($baoxiu['status']); ?></a>
							<a class="confirm ajax-get" title="删除" href="<?php echo url('del?id='.$baoxiu['id']); ?>">删除</a>
						</td>
					</tr>
